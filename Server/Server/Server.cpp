...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		if (!gServerObj.isConnected()) {
			MessageBox(0, L"Failed to initialise server socket.", 0, 0);
			_getch();
			return 1;
		}
    ...
    case WM_COMMAND:
        {
			...
			case ID_USER_CONNECT:
			{
                ...
                /*for (auto clie : gServerObj.getClientList())
                {
                    wcscat(buffer, clie->username.c_str());
                    wcscat(buffer, L",  ");
                }*/
...
void RestoreData()
{
	...
	fos.imbue(loc);//cần inbue lớp locale để ghi ký tự unicode
...
void LoadData()
{
	...
	while (!fin.eof()) {
		...
		for (k = j; k < len; k++) password[k - j] = buffer[k];
